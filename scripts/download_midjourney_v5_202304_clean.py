...
def main():
    ...
    args.add_argument("--upscaled_dataset", action="store_true")
    args = args.parse_args()

    # download parquet from https://huggingface.co/datasets/wanng/midjourney-v5-202304-clean

    if args.upscaled_dataset:
        # single upscaled image
        data = pd.read_parquet("/fsx/william/tmp/upscaled_prompts_df.parquet")

        write_to = "pipe:aws s3 cp - s3://muse-datasets/midjourney_v5_202304_clean_upscaled"

        checkpoint_dir = "/fsx/william/tmp/download_midjourney_v5_202304_clean_upscaled"
    else:
        # 4x4 grid
        data = pd.read_parquet("/fsx/william/tmp/ori_prompts_df.parquet")

        write_to = "pipe:aws s3 cp - s3://muse-datasets/midjourney_v5_202304_clean"

        checkpoint_dir = "/fsx/william/tmp/download_midjourney_v5_202304_clean"
